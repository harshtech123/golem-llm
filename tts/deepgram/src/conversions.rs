use crate::client::{
   Model, TextToSpeechParams, TextToSpeechRequest, 
};
...
pub fn deepgram_model_to_voice_info(model: Model) -> VoiceInfo {
    ...
    VoiceInfo {
        ...
        gender,
        quality,
        description: Some(format!("{} voice with {} accent, {}. Characteristics: {}. Suitable for: {}",
            model.gender,
            model.accent,
            model.age,
...
pub fn infer_quality_from_model(voice_id: &str) -> VoiceQuality {
    ...
    } else if voice_id.starts_with("aura-") {
        VoiceQuality::Standard // Aura-1 is the standard quality
    } else {
...
pub fn synthesis_options_to_tts_request(
    ...
    if let Some(opts) = options {
        ...
        if let Some(audio_config) = opts.audio_config {
            let (encoding, container, sample_rate, bit_rate) = audio_format_to_deepgram_params(audio_config.format);
...
fn audio_format_to_deepgram_params(format: AudioFormat) -> (String, Option<String>, u32, Option<u32>) {
    match format {
        AudioFormat::Mp3 => ("mp3".to_string(), None, 22050, Some(48000)),
        AudioFormat::Wav => ("linear16".to_string(), Some("wav".to_string()), 24000, None),
        AudioFormat::Pcm => ("linear16".to_string(), Some("none".to_string()), 24000, None),
        AudioFormat::OggOpus => ("opus".to_string(), Some("ogg".to_string()), 48000, Some(12000)),
...
pub fn audio_data_to_synthesis_result(
    ...
    let metadata = SynthesisMetadata {
        ...
        provider_info: Some(format!("Deepgram TTS - Encoding: {}, Sample Rate: {}Hz", encoding, sample_rate)),
    };
...
pub fn validate_text_input(text: &str, model: Option<&str>) -> ValidationResult {
    ...
    if character_count > max_chars {
        errors.push(format!("Text exceeds maximum character limit of {} characters (current: {})", max_chars, character_count));
    }
...
mod tests {
    ...
    fn test_audio_format_conversion() {
        let (encoding, container, sample_rate, _) = audio_format_to_deepgram_params(AudioFormat::Mp3);
